...
module.exports = function (app) {
 ...
 app.route('/api/check')
    .post((req, res) => {
      ...
      let legitPuzzle = solver.validate(puzzle); 
      if (!puzzle || !coordinate || !value){
        res.json({error:'Required field(s) missing'})
      }
      else if (legitPuzzle === "Invalid characters in puzzle"){
        res.json({error:"Invalid characters in puzzle"})
      } 
      else if (legitPuzzle === "Expected puzzle to be 81 characters long"){
        ...
        let board = solver.transformToBoard(legitPuzzle);
        let conflict = []; 
  ...
  app.route('/api/solve')
    .post((req, res) => {
      ...
      let legitPuzzle = solver.validate(puzzle);
      if (legitPuzzle === "Required field missing"){
        ...
        let solution = solver.solve(legitPuzzle); 
